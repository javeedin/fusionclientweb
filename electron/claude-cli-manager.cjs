...
// usage: node tools/load-db.cjs <file.json> <table_name>
let DatabaseSync;
try { ({ DatabaseSync } = require('node:sqlite')); }
catch (e) { console.error('node:sqlite unavailable — Node 22+ required (found ' + process.version + ')'); process.exit(1); }
const fs = require('fs');
const path = require('path');
const [file, tableArg] = process.argv.slice(2);
if (!file || !tableArg) { console.error('usage: node tools/load-db.cjs <file.json> <table>'); process.exit(1); }
...
const ins = db.prepare('INSERT INTO ' + q(table) + ' VALUES (' + cols.map(() => '?').join(',') + ')');
db.exec('BEGIN');
for (const r of rows) {
  ins.run(...cols.map(c => {
    const v = (r || {})[c];
...
db.exec('COMMIT');
console.log('table ' + table + ': ' + rows.length + ' rows loaded into erp-data.db');
