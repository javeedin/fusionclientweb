...
function provisionWorkspace(ctx = {}) {
  ...
  // Direct REST access (like the in-app assistant's erp_api_get/write):
  // a pre-approved script that calls the company's ORDS API with the same
  // credentials/token handling the app uses. Writes need an explicit
  // --confirm flag, and CLAUDE.md instructs Claude to get the user's OK in
  // chat first — the conversational replacement for the approval card.
  const apexFullBase = (ctx.apexBaseUrl && String(ctx.apexBaseUrl).trim())
    || `${env.ORACLE_BASE_URL.replace(/\/+$/, '')}/ords/bcldifc/reerp`;
  fs.mkdirSync(path.join(ws, 'tools'), { recursive: true });
  fs.writeFileSync(path.join(ws, 'tools', 'call-api.cjs'), `// generated by Re-ERP — call the ERP ORDS REST API directly
// usage: node tools/call-api.cjs GET  "/gl/periodsstatus"
//        node tools/call-api.cjs GET  "/cash/externaltransactions?date_from=2026-06-01&date_to=2026-06-30"
//        node tools/call-api.cjs POST "/ai/training" body.json --confirm   (writes need --confirm)
const fs = require('fs');
const ENV = ${JSON.stringify({
    ORACLE_BASE_URL: env.ORACLE_BASE_URL,
    ORACLE_USERNAME: env.ORACLE_USERNAME,
    ORACLE_PASSWORD: env.ORACLE_PASSWORD,
    SKIP_AUTH: env.SKIP_AUTH,
    ...(env.ORDS_USE_TOKEN ? { ORDS_USE_TOKEN: env.ORDS_USE_TOKEN } : {}),
    ...(env.ORDS_CLIENT_ID ? { ORDS_CLIENT_ID: env.ORDS_CLIENT_ID } : {}),
    ...(env.ORDS_CLIENT_SECRET ? { ORDS_CLIENT_SECRET: env.ORDS_CLIENT_SECRET } : {}),
  })};
...
`, 'utf8');
  ...
  // Local SQL analysis: JSON pulls → SQLite (Node's built-in node:sqlite,
  // zero dependencies) so Claude answers with exact SQL instead of re-reading
  // big JSON files or re-querying the API. query-db opens read-only.
  fs.writeFileSync(path.join(ws, 'tools', 'load-db.cjs'), `// generated by Re-ERP — load a JSON pull into erp-data.db for SQL analysis
// usage: node tools/load-db.cjs <file.json> <table_name>            (replace)
//        node tools/load-db.cjs <file.json> <table_name> --append   (add rows;
//        CAUTION: appending overlapping pulls double-counts rows)
let DatabaseSync;
try { ({ DatabaseSync } = require('node:sqlite')); }
catch (e) { console.error('node:sqlite unavailable — Node 22+ required (found ' + process.version + ')'); process.exit(1); }
const fs = require('fs');
const path = require('path');
const args = process.argv.slice(2);
const append = args.includes('--append');
const [file, tableArg] = args.filter(a => a !== '--append');
if (!file || !tableArg) { console.error('usage: node tools/load-db.cjs <file.json> <table> [--append]'); process.exit(1); }
const table = String(tableArg).replace(/[^A-Za-z0-9_]/g, '_');
const text = fs.readFileSync(file, 'utf8');
let data;
try { data = JSON.parse(text); }
catch (e) { data = JSON.parse(text.replace(/:(\\s*-?\\d+)\\.(?=\\s*[,}\\]])/g, ':$1')); } // Oracle trailing-dot numbers
const rows = Array.isArray(data) ? data : Array.isArray(data.items) ? data.items : Array.isArray(data.rows) ? data.rows : null;
if (!rows || !rows.length) { console.error('no rows found (expected {items:[...]} or a JSON array)'); process.exit(1); }
const cols = [];
const seen = new Set();
rows.slice(0, 500).forEach(r => Object.keys(r || {}).forEach(k => { if (!seen.has(k)) { seen.add(k); cols.push(k); } }));
const q = (s) => '"' + String(s).replace(/"/g, '""') + '"';
const db = new DatabaseSync(path.join(__dirname, '..', 'erp-data.db'));
const exists = !!db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name = ?").get(table);
let useCols = cols;
if (append && exists) {
  // keep the existing schema; unknown incoming keys are dropped, missing ones null
  useCols = db.prepare('SELECT name FROM pragma_table_info(?)').all(table).map(c => c.name);
} else {
  db.exec('DROP TABLE IF EXISTS ' + q(table));
  db.exec('CREATE TABLE ' + q(table) + ' (' + cols.map(q).join(', ') + ')');
}
const ins = db.prepare('INSERT INTO ' + q(table) + ' VALUES (' + useCols.map(() => '?').join(',') + ')');
db.exec('BEGIN');
for (const r of rows) {
  ins.run(...useCols.map(c => {
    const v = (r || {})[c];
...
db.exec('COMMIT');
const total = db.prepare('SELECT COUNT(*) AS n FROM ' + q(table)).get().n;
console.log('table ' + table + ': ' + rows.length + ' rows ' + (append && exists ? 'appended' : 'loaded') + ' — table now has ' + total + ' rows');
console.log('columns: ' + useCols.join(', '));
`, 'utf8');

  fs.writeFileSync(path.join(ws, 'tools', 'query-db.cjs'), `// generated by Re-ERP — query the local analysis database (read-only)
// usage: node tools/query-db.cjs "SELECT ..."   (results print as JSON)
//        node tools/query-db.cjs --tables       (tables, columns, row counts)
let DatabaseSync;
try { ({ DatabaseSync } = require('node:sqlite')); }
catch (e) { console.error('node:sqlite unavailable — Node 22+ required (found ' + process.version + ')'); process.exit(1); }
const fs = require('fs');
const path = require('path');
const DB = path.join(__dirname, '..', 'erp-data.db');
if (!fs.existsSync(DB)) { console.error('erp-data.db not found — load data first: node tools/load-db.cjs <file.json> <table>'); process.exit(1); }
const db = new DatabaseSync(DB, { readOnly: true });
const arg = process.argv[2];
if (!arg) { console.error('usage: node tools/query-db.cjs "SELECT ..." | --tables'); process.exit(1); }
const q = (s) => '"' + String(s).replace(/"/g, '""') + '"';
if (arg === '--tables') {
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all();
  const out = tables.map(t => ({
    name: t.name,
    rows: db.prepare('SELECT COUNT(*) AS n FROM ' + q(t.name)).get().n,
    columns: db.prepare('SELECT name FROM pragma_table_info(?)').all(t.name).map(c => c.name),
  }));
  console.log(JSON.stringify({ tables: out }));
} else {
  const rows = db.prepare(arg).all();
  const s = JSON.stringify({ rowCount: rows.length, rows });
  if (s.length > 2000000) { console.error('Result over 2MB — add a LIMIT or aggregate'); process.exit(2); }
  console.log(s);
}
`, 'utf8');
...
## Direct REST API (no MCP needed)
You can call the ERP's ORDS REST API directly (pre-approved command; auth is
handled inside the script):
  node tools/call-api.cjs GET "/gl/periodsstatus"
  node tools/call-api.cjs GET "/cash/externaltransactions?date_from=2026-06-01&date_to=2026-06-30&row_limit=500"
Useful GET endpoints (responses are {items:[...]}):
- GL: /gl/journals/headers?p_offset=0&p_limit=50 · /gl/journals/lines?<filters>
      /gl/rr-trialbalance/standard?ledger_name=&period_name=&account= (period like Jun-26)
      /gl/periodsstatus · /glaccountslist · /accountanalysis?<filters> · /gl/businessunits
- AP: /ap/invoices/stats · /ap/invoices/outstanding-by-supplier · /ap/invoices/aging-data
      /ap/payments?limit=100 · /suppliers?limit=200 · /suppliers/balance/invoices/{supplierNumber}
- AR: /ar/invoices?<filters> · /ar/receipts?<filters> · /ar/customers · /ar/invoice-balances
- Cash: /cash/bankstatements · /cash/externaltransactions?row_limit=500 · /banks/bankaccounts
...
## Local SQL analysis (erp-data.db)
For analysis, aggregation and CROSS-SOURCE JOINS, load pulls into the local
SQLite database and answer with SQL (exact, instant, no tokens wasted):
  node tools/load-db.cjs extl.json external_transactions
  node tools/load-db.cjs extl2.json external_transactions --append   (append instead of replace — only for non-overlapping pulls, e.g. another month; overlapping data double-counts)
  node tools/query-db.cjs --tables
  node tools/query-db.cjs "SELECT businessUnitName, SUM(amount) FROM external_transactions GROUP BY 1"
query-db is read-only and prints JSON. PREFER this over re-reading big JSON
files or repeated API calls: fetch once (--out), load-db once, then every
follow-up question is one SELECT — including joins across tables loaded from
different endpoints (transactions vs journal lines, invoices vs payments).
Check --tables first when unsure of the schema; use LIMIT on exploratory
SELECT *. The user can also load data and browse tables from the app's
Preview panel — tables you find in --tables may have been loaded by them.
...
WRITES (POST/PUT/DELETE): allowed only with the --confirm flag, e.g.
  node tools/call-api.cjs POST "/gl/journals/create" body.json --confirm
NEVER use --confirm unless the user explicitly confirmed that exact write in
this conversation (show them method, endpoint and full JSON body first and
wait for their yes). Fetch and validate before proposing any write.
