-- ============================================================
-- PATCH 131: Add external_transaction_id filter to the
--            GET cash/externaltransactions handler
--
-- Problem:
--   The Petty Cash "Bank Txn ID" drill (and other single-record
--   lookups) call
--       GET cash/externaltransactions?external_transaction_id=<id>&row_limit=1
--   but the GET handler (patch 93) has NO external_transaction_id
--   bind — the parameter was ignored, so the drill never returned
--   the intended row and the "Bank Transaction" dialog showed
--   "Bank transaction not found" even though batchstatus finds it.
--
-- Fix:
--   Re-define the GET handler identically to patch 93, plus:
--     l_ext_id declaration bound to :external_transaction_id, and
--     AND (l_ext_id IS NULL OR EXTERNAL_TRANSACTION_ID = TO_NUMBER(l_ext_id))
--   Everything else (STATUS derivation, columns, JSON keys) is
...
BEGIN
  ...
  ORDS.DEFINE_HANDLER(
    ...
    p_items_per_page => 0,
    p_mimes_allowed  => '',
    p_comments       => 'Query external cash transactions — supports external_transaction_id filter',
    p_source         => q'[
DECLARE
    l_ext_id       VARCHAR2(30)  := :external_transaction_id;
    ...
    CURSOR c_txns IS
        SELECT EXTERNAL_TRANSACTION_ID,
         ...
         WHERE (l_ext_id       IS NULL OR EXTERNAL_TRANSACTION_ID = TO_NUMBER(l_ext_id))
...
-- VERIFY:
--   GET {base}/cash/externaltransactions?external_transaction_id=1000000395&row_limit=1
--   → returns the single matching row (items[0]); the Petty Cash Bank Txn ID
--     drill then opens it instead of "Bank transaction not found".
