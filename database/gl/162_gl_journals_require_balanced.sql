-- ============================================================
-- 162_gl_journals_require_balanced.sql
-- RR_INSERT_GL_JOURNALS_POST (POST /journals/create) now REJECTS any
-- journal whose debits do not equal its credits — nothing is inserted.
--
-- Every module creates GL journals through this one procedure (payments,
-- invoices, manual journals, bank, fixed assets, revaluation, …), so this
-- is the single place that guarantees the ledger can never go out of
-- balance again (e.g. the 850.00 payment 000176 journal #1000002471).
--
-- Check uses the SAME accounted amounts the procedure stores:
--   normal journals      accounted = ROUND(entered × rate, 2) per line
--   REVALUATION journals accounted = accountedDr/Cr from the payload
-- Rules:
--   Σ accounted Dr = Σ accounted Cr                → posted as before
--   foreign-currency journal (a line rate ≠ 1) off by ≤ 0.01 per line
--     (per-line rounding)                           → difference absorbed
--     into the largest line, like an Oracle rounding adjustment; reported
--     back as roundingAdjustment
--   anything else                                   → rejected with
--     status ERROR, validationError UNBALANCED_JOURNAL, totals in the
--     message, logged to RR_GL_VALIDATION_LOG
--
-- Base: 112_fix_journals_create_varchar_limit.sql (identical otherwise).
-- Run the whole file in the bcldifc schema.
-- ============================================================
...
AS
    ...
    -- Debit = credit pre-validation
    v_n_lines               PLS_INTEGER;
    v_chk_rate              NUMBER;
    v_chk_dr                NUMBER;
    v_chk_cr                NUMBER;
    v_bal_dr                NUMBER := 0;
    v_bal_cr                NUMBER := 0;
    v_bal_diff              NUMBER := 0;
    v_chk_fx                BOOLEAN := FALSE;
    v_adj_idx               PLS_INTEGER := -1;
    v_adj_side              VARCHAR2(2);
    v_adj_max               NUMBER := -1;

    -- *** FIX: use get_clob() to avoid ORA-40478 when value > 4000 chars ***
...
BEGIN
    ...
    -- ── PRE-VALIDATION: debits must equal credits (as they will be stored) ──
    l_step := 'Pre-validating debit = credit';
    v_n_lines := l_lines_arr.get_size();
    FOR i IN 0 .. v_n_lines - 1 LOOP
        l_line_obj := JSON_OBJECT_T(l_lines_arr.get(i));
        v_chk_rate := NVL(safe_get_number(l_line_obj, 'currencyConversionRate'),
                          NVL(safe_get_number(l_header_obj, 'currencyConversionRate'), 1));
        IF v_chk_rate IS NULL OR v_chk_rate <= 0 THEN
            v_chk_rate := 1;
        END IF;
        IF v_chk_rate != 1 THEN
            v_chk_fx := TRUE;
        END IF;
        IF UPPER(NVL(v_header_je_category, '')) = 'REVALUATION' THEN
            v_chk_dr := NVL(safe_get_number(l_line_obj, 'accountedDr'),
                            ROUND(NVL(safe_get_number(l_line_obj, 'enteredDr'), 0) * v_chk_rate, 2));
            v_chk_cr := NVL(safe_get_number(l_line_obj, 'accountedCr'),
                            ROUND(NVL(safe_get_number(l_line_obj, 'enteredCr'), 0) * v_chk_rate, 2));
        ELSE
            v_chk_dr := ROUND(NVL(safe_get_number(l_line_obj, 'enteredDr'), 0) * v_chk_rate, 2);
            v_chk_cr := ROUND(NVL(safe_get_number(l_line_obj, 'enteredCr'), 0) * v_chk_rate, 2);
        END IF;
        v_bal_dr := v_bal_dr + v_chk_dr;
        v_bal_cr := v_bal_cr + v_chk_cr;
        -- largest line: takes a rounding adjustment if one is needed
        IF ABS(v_chk_dr) > v_adj_max THEN
            v_adj_max := ABS(v_chk_dr); v_adj_idx := i; v_adj_side := 'DR';
        END IF;
        IF ABS(v_chk_cr) > v_adj_max THEN
            v_adj_max := ABS(v_chk_cr); v_adj_idx := i; v_adj_side := 'CR';
        END IF;
    END LOOP;
    v_bal_diff := ROUND(v_bal_dr - v_bal_cr, 2);

    IF v_bal_diff = 0 THEN
        v_adj_idx := -1;                       -- balanced: no adjustment
    ELSIF v_chk_fx AND ABS(v_bal_diff) <= 0.01 * v_n_lines THEN
        NULL;                                  -- FX rounding: absorbed below
    ELSE
        BEGIN
            INSERT INTO RR_GL_VALIDATION_LOG (
                LOG_ID, MODULE, REFERENCE_NO, BATCH_NAME,
                ...
                'FAILED', 1,
                'UNBALANCED_JOURNAL',
                'Journal rejected: debits ' || TO_CHAR(v_bal_dr, 'FM999G999G999G990D00')
                    || ' <> credits ' || TO_CHAR(v_bal_cr, 'FM999G999G999G990D00')
                    || ' (difference ' || TO_CHAR(v_bal_diff, 'FM999G999G999G990D00') || ').',
                TO_CLOB('{"totalDr":' || TO_CHAR(v_bal_dr, 'TM9') || ',"totalCr":' || TO_CHAR(v_bal_cr, 'TM9')
                    || ',"difference":' || TO_CHAR(v_bal_diff, 'TM9') || ',"lines":' || v_n_lines || '}'),
                p_json_input,
                v_batch_created_by
            );
            COMMIT;
        ...
        l_result_obj.put('status',          'ERROR');
        l_result_obj.put('validationError', 'UNBALANCED_JOURNAL');
        l_result_obj.put('totalDr',         v_bal_dr);
        l_result_obj.put('totalCr',         v_bal_cr);
        l_result_obj.put('difference',      v_bal_diff);
        l_result_obj.put('message',
            'Journal rejected: debits (' || TO_CHAR(v_bal_dr, 'FM999G999G999G990D00')
            || ') do not equal credits (' || TO_CHAR(v_bal_cr, 'FM999G999G999G990D00')
            || '), difference ' || TO_CHAR(v_bal_diff, 'FM999G999G999G990D00')
            || '. Nothing was posted. Check for lines with a missing or negative amount '
            || '(e.g. credit notes) and re-create the accounting.');
        p_json_output := l_result_obj.to_clob();
        RETURN;
    END IF;
    ...
    -- Always "Manual - <batch_id>" for screen-created journals
    ...
    FOR i IN 0 .. l_lines_arr.get_size() - 1 LOOP
        ...
        -- FX rounding adjustment (validated above): keeps the stored journal balanced
        IF i = v_adj_idx THEN
            IF v_adj_side = 'DR' THEN
                v_line_accounted_dr := v_line_accounted_dr - v_bal_diff;
            ELSE
                v_line_accounted_cr := v_line_accounted_cr + v_bal_diff;
            END IF;
        END IF;
    ...
    IF v_adj_idx >= 0 THEN
        l_result_obj.put('roundingAdjustment', v_bal_diff);
        l_result_obj.put('roundingAdjustedLine', v_adj_idx + 1);
    END IF;
    p_json_output := l_result_obj.to_clob();
...
SELECT line, position, text FROM user_errors WHERE name = 'RR_INSERT_GL_JOURNALS_POST' ORDER BY sequence;
