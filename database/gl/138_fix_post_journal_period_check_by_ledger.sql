-- ============================================================
-- Fix RR_POST_JOURNAL: period-open check must respect the ledger
--
-- Problem:
--   The period check queried RR_ACCOUNTING_PERIODS_STATUS with
--   only PERIOD_NAME_ID + APPLICATION_ID and ROWNUM = 1 — no
--   LEDGER_ID filter. The table holds one row per
--   (PERIOD_NAME_ID, APPLICATION_ID, LEDGER_ID), so ROWNUM = 1
--   returned an arbitrary ledger's row. A period genuinely Open
--   for the journal's ledger could be reported as "Never Opened"
--   because another ledger's row was picked instead.
--
--   Also, PERIOD_NAME_ID is stored in two formats:
--     'Aug-26'      (plain)
--     '16_Aug-26'   (set_num prefix, Fusion sync)
--   The check matched only the plain form, while the open/close
--   endpoint (15_period_status_update.sql) updates both.
--
-- Fixes applied (period check only; everything else unchanged
-- from 53_fix_rr_post_journal_revaluation_zero_check.sql):
--   1. Read LEDGER_ID from the journal header (fallback: batch).
--   2. Filter the period-status lookup by that LEDGER_ID.
--   3. Match PERIOD_NAME_ID exactly OR with the '%_<period>'
--      suffix pattern, same as the open/close endpoint.
--   4. Period counts as Open when ANY matching row for the
--      ledger has CLOSING_STATUS = 'O' (both name formats may
...
BEGIN
    ...
    -- ── 2. Period name, JE category + ledger from the first journal header ────
    BEGIN
        SELECT PERIOD_NAME,
               NVL(UPPER(USER_JE_CATEGORY_NAME), ''),
               LEDGER_ID
        INTO   v_period_name, v_je_category, v_ledger_id
    ...
    -- One row per (PERIOD_NAME_ID, APPLICATION_ID, LEDGER_ID); PERIOD_NAME_ID
    -- may be plain ('Aug-26') or prefixed ('16_Aug-26'), so match both — the
    -- same pattern the open/close endpoint uses. Open when any matching row
    -- for the ledger is 'O'.
    ...
    THEN
        ...
        AND    (v_ledger_id IS NULL OR LEDGER_ID = v_ledger_id);

        IF v_period_rows = 0 THEN
            add_error('Period "' || v_period_name ||
                '" was not found in the period status table' ||
                CASE WHEN v_ledger_id IS NOT NULL
                     THEN ' for ledger ' || v_ledger_id ELSE '' END ||
...
DBMS_OUTPUT.PUT_LINE('RR_POST_JOURNAL updated — period-open check now filters by the journal''s LEDGER_ID and matches both PERIOD_NAME_ID formats');
