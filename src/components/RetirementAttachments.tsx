// Retirement attachments panel — mirror of PaymentAttachments.tsx, keyed by
// the retirement's RETIREMENT_ID. Same upload/download/delete UX as payments.
...
import {
  listRetirementAttachments, uploadRetirementAttachment, downloadRetirementAttachment, deleteRetirementAttachment,
  formatFileSize, fileIcon,
  type RetirementAttachment,
} from '../services/retirementAttachment.service';
...
interface Props {
  retirementId:   number | string | null | undefined;
  readOnly?:      boolean;
...
const RetirementAttachments: React.FC<Props> = ({ retirementId, readOnly = false, onCountChange }) => {
  const { user } = useAuth();

  const [attachments,  setAttachments]  = useState<RetirementAttachment[]>([]);
  ...
  const load = useCallback(async () => {
    if (retirementId === null || retirementId === undefined || retirementId === '') return;
    setLoading(true);
    setFetchError(null);
    try {
      const rows = await listRetirementAttachments(retirementId);
      setAttachments(rows);
  ...
  const handleUpload: UploadProps['customRequest'] = async (options) => {
    ...
    const result = await uploadRetirementAttachment(
      retirementId!,
      file,
  ...
  // ── Download ─────────────────────────────────────────────────────────────
  const handleDownload = async (rec: RetirementAttachment) => {
    setDownloading(prev => new Set([...prev, rec.attachmentId]));
    try {
      await downloadRetirementAttachment(retirementId!, rec.attachmentId, rec.fileName);
    } catch (e: any) {
  ...
  // ── Delete ───────────────────────────────────────────────────────────────
  const handleDelete = async (rec: RetirementAttachment) => {
    setDeleting(prev => new Set([...prev, rec.attachmentId]));
    const result = await deleteRetirementAttachment(
      retirementId!,
      rec.attachmentId,
      user?.name || user?.username || 'System',
    );
  ...
  // ── Table columns ─────────────────────────────────────────────────────────
  const columns: ColumnsType<RetirementAttachment> = [
    {
      title: '',
      width: 30,
      render: (_, rec) => (
  ...
  if (retirementId === null || retirementId === undefined || retirementId === '') {
    return (
      <Alert
        type="info"
        showIcon
        message="Save the retirement first to enable attachments"
        style={{ margin: '12px 0' }}
      />
    );
  }
...
export default RetirementAttachments;
