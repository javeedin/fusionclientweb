...
/**
 * Review-and-save dialog for a captured training recipe. Used by the
 * per-page TeachAIButton and by the global Teach AI capture button.
 */
export const TeachAIModal: React.FC<{
  open: boolean;
  initial: TrainingRecipe | null;
  onClose: () => void;
  header?: React.ReactNode;   // optional extra content above the fields
}> = ({ open, initial, onClose, header }) => {
  const [recipe, setRecipe] = useState<TrainingRecipe | null>(initial);
  const [saving, setSaving] = useState(false);

  useEffect(() => { setRecipe(initial); }, [initial, open]);
 ...
 * Pages pass a buildRecipe() that snapshots their current request.
 */
const TeachAIButton: React.FC<{
  ...
  return (
    <>
      ...
      <TeachAIModal open={open} initial={recipe} onClose={() => setOpen(false)} />
    </>
  );
};
