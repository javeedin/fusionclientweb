// Asset (addition) attachments panel — mirror of RetirementAttachments.tsx,
// keyed by the asset's ASSET_ID. Same upload/download/delete UX.
...
import {
  listAssetAttachments, uploadAssetAttachment, downloadAssetAttachment, deleteAssetAttachment,
  formatFileSize, fileIcon,
  type AssetAttachment,
} from '../services/assetAttachment.service';
...
const AssetAttachments: React.FC<Props> = ({ assetId, readOnly = false, onCountChange }) => {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const rows = await listAssetAttachments(assetId);
      setAttachments(rows);
  ...
  const handleUpload: UploadProps['customRequest'] = async (options) => {
    ...
    const result = await uploadAssetAttachment(
      assetId!,
      file,
  ...
  // ── Download ─────────────────────────────────────────────────────────────
  const handleDownload = async (rec: AssetAttachment) => {
    setDownloading(prev => new Set([...prev, rec.attachmentId]));
    try {
      await downloadAssetAttachment(assetId!, rec.attachmentId, rec.fileName);
    } catch (e: any) {
  ...
  // ── Delete ───────────────────────────────────────────────────────────────
  const handleDelete = async (rec: AssetAttachment) => {
    setDeleting(prev => new Set([...prev, rec.attachmentId]));
    const result = await deleteAssetAttachment(
      assetId!,
      rec.attachmentId,
      user?.name || user?.username || 'System',
    );
  ...
  // ── Table columns ─────────────────────────────────────────────────────────
  const columns: ColumnsType<AssetAttachment> = [
    {
      title: '',
      width: 30,
      render: (_, rec) => (
  ...
  if (assetId === null || assetId === undefined || assetId === '') {
    return (
      <Alert
        type="info"
        showIcon
        message="Save the asset first to enable attachments"
        style={{ margin: '12px 0' }}
      />
    );
  }
