...
import {
  ...
  message, Empty, Divider, Badge, Segmented, Modal, Checkbox, Steps, Popover, Spin,
} from 'antd';
...
const UnreconciledTab: React.FC<UnreconciledTabProps> = ({ bankAccounts, businessUnits, loadingAccounts }) => {
  ...
  const [autoReconStep,     setAutoReconStep]     = useState<'select' | 'results'>('select');
  ...
  const runAutoRecon = useCallback(async () => {
    ...
    setAutoReconMatches(matches);
    setAutoReconStep('select');
    setAutoReconRunning(false);
    setSelectedStmtForRecon(new Set());
    setSelectedSysForRecon(new Set());
    ...
    <>
      ...
      >
        {(() => {
          ...
          const [loadingMatching, setLoadingMatching] = React.useState(false);

          // Auto-fetch system transactions when modal opens if not loaded
          React.useEffect(() => {
            const doFetch = async () => {
              // If sysTxns already loaded, use that
              if (sysTxns.length > 0) {
                setMatchingSysTxns(sysTxns);
                return;
              }

              // Otherwise, fetch unreconciled system transactions
